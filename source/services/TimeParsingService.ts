export class TimeParsingService {
	 ...
	 */
	static parseTimeToHours(timeStr: string): number {
		if (!timeStr) return 1;
	 ...
	 */
	static normalizeTimeString(inputValue: string): string {
	 ...
	 */
	static generateTimeMarks(incrementMinutes: number): number[] {
		const marks = [];
	...
	static adjustTime(
		...
		const currentHours = this.parseTimeToHours(currentTimeStr);
		const totalMinutes = Math.round(currentHours * 60);

		const marks = this.generateTimeMarks(incrementMinutes);
